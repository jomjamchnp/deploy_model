...
def checkcrash(data_num,x1,y1,x2,y2,typehand,h,k,r,rec,output):
    ...
    while(math.pow((newX1-h),2) + math.pow((newY1-k),2) <= math.pow((r),2)):
        ...
        status,number=inArea((int(newX1),int(newY1)),data_num,rec)
    ...
    while(math.pow((newX2-h),2) + math.pow((newY2-k),2) <= math.pow((r),2)):
        ...
        #print((int(newX),int(newY)))
        status,number=inArea((int(newX2),int(newY2)),data_num,rec)
...
def inArea(p,data,rec) : #,n,x1,y1,x2,y2
    status = False
    number = 0
    for i in data:
...
def checkline(h,k,xhead,yhead,xtail,ytail,rec,data_num,r,typehand):
    ...
    while(math.pow((newX-h),2) + math.pow((newY-k),2) <= math.pow((r),2)):
        ...
        #print((int(newX),int(newY)))
        status,number=inArea((int(newX),int(newY)),data_num,rec)
        if(status):
    ...
    if(len(data)==2):
        for i in range(0, len(data)):
            ...
            if(name[0]=="arrownohead"):
                distance,x1,y1,x2,y2 = arrownohead(xmin,ymin,lenx,leny,data_corr,img)
                list_distance.append(distance)
                box_arrow.append((x1+xmin,y1+ymin,x2+xmin,y2+ymin,name[0]))
            if(name[0]=="arrow"):     
                distance,(p1,p2),center = arrow(xmin,ymin,lenx,leny,data_corr,img)
                x,y = center
                list_distance.append(distance)
...
def score_hand(name,img_hands):    
    ...
    #read hand
    with open("./object_detection/json_num/script_hand.json") as f:
        data = json.load(f)
    
    data_corr = []
    data_circle = []
    # read num from json
    with open("./object_detection/json_num/script_number.json") as f:
        data2 = json.load(f)
